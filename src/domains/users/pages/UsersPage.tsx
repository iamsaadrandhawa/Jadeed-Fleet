    ...
    <div className="space-y-6">
            ...
            <div className="hidden overflow-x-auto md:block">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                  {users.map((u) => (
                    <tr key={u.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800 dark:bg-neutral-800">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-neutral-900 dark:text-neutral-100">{u.full_name ?? "—"}</p>
                            <p className="text-xs text-neutral-500 dark:text-neutral-400 dark:text-neutral-500">{u.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-neutral-700 dark:text-neutral-300">{u.role?.name ?? "—"}</td>
                      <td className="px-4 py-3">
                        <Badge tone={statusTone(u.status)} dot>{u.status}</Badge>
                      </td>
                      <td className="px-4 py-3 text-neutral-500 dark:text-neutral-400 dark:text-neutral-500">{formatDate(u.created_at)}</td>
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end gap-1">
                          {canUpdate && (
                            <button onClick={() => openEdit(u)} className="rounded-md p-1.5 text-neutral-400 dark:text-neutral-500 hover:bg-neutral-100 dark:hover:bg-neutral-800 dark:bg-neutral-800 hover:text-neutral-900 dark:hover:text-neutral-100 dark:text-neutral-100">
                              <Pencil size={16} />
                            </button>
                          )}
                          {canDelete && (
                            <button onClick={() => handleDelete(u)} className="rounded-md p-1.5 text-neutral-400 dark:text-neutral-500 hover:bg-red-50 dark:hover:bg-red-950/50 hover:text-red-600 dark:hover:text-red-400 dark:text-red-400">
                              <Trash2 size={16} />
                            </button>
                          )}
                        </div>
              ...
              {users.map((u) => (
                <div key={u.id} className="px-4 py-3">
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    <span className="text-xs text-neutral-500 dark:text-neutral-400 dark:text-neutral-500">{u.role?.name ?? "—"}</span>
                    <div className="flex gap-1">
                      {canUpdate && (
                        <button onClick={() => openEdit(u)} className="rounded-md p-1.5 text-neutral-400 dark:text-neutral-500 hover:bg-neutral-100 dark:hover:bg-neutral-800 dark:bg-neutral-800">
                          <Pencil size={16} />
                        </button>
                      )}
                      {canDelete && (
      ...
      >
        <form onSubmit={handleSave} className="space-y-4">
          <Input label="Email" type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} disabled={!!editingUser} />
          <Input
